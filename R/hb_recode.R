...
hb_recode <- function(x, recode_to = "long", and_choice = "and") {
  ...
  if (recode_to == "short") {
    hb_out <- dplyr::case_when(
      x %in% c("S08000001", "S08000015", "AA", "AYRSHIRE AND ARRAN", "AYRSHIRE & ARRAN") ~ "AA", 
      x %in% c("S08000002", "S08000016", "BR", "BOR", "BORDERS") ~ "BR",
      x %in% c("S08000003", "S08000017", "DG", "DUMFRIES AND GALLOWAY", "DUMFRIES & GALLOWAY") ~ "DG",
      x %in% c("S08000004", "S08000018", "S08000029", "FF", "FIFE") ~ "FF",
      x %in% c("S08000005", "S08000019", "FV", "FORTH VALLEY") ~ "FV",
      x %in% c("S08000007", "S08000021", "S08000031", "GGC", "GC", "GG", "GGHB", "GREATER GLASGOW AND CLYDE", "GREATER GLASGOW & CLYDE") ~ "GC",
      x %in% c("S08000006", "S08000020", "GR", "GRA", "GRAMPIAN") ~ "GR",
      x %in% c("S08000008", "S08000022", "HG", "HIG", "HIGHLAND", "HIGHLANDS") ~ "HG",
      x %in% c("S08000009", "S08000023", "S08000032", "LN", "LAN", "LANARKSHIRE") ~ "LN",
      ...
      x %in% c("S08000013", "S08000027", "S08000030", "TY", "TAY", "TAYSIDE") ~ "TY",
    ...
    hb_out <- dplyr::case_when(
      x %in% c("S08000001", "S08000015", "AA", "AYRSHIRE AND ARRAN", "AYRSHIRE & ARRAN") ~ "Ayrshire and Arran", 
      x %in% c("S08000002", "S08000016", "BR", "BOR", "BORDERS") ~ "Borders",
      x %in% c("S08000003", "S08000017", "DG", "DUMFRIES AND GALLOWAY", "DUMFRIES & GALLOWAY") ~ "Dumfries and Galloway",
      x %in% c("S08000004", "S08000018", "S08000029", "FF", "FIFE") ~ "Fife",
      x %in% c("S08000005", "S08000019", "FV", "FORTH VALLEY") ~ "Forth Valley",
      x %in% c("S08000007", "S08000021", "S08000031", "GGC", "GC", "GG", "GGHB", "GREATER GLASGOW AND CLYDE", "GREATER GLASGOW & CLYDE") ~ "Greater Glasgow and Clyde",
      x %in% c("S08000006", "S08000020", "GR", "GRA", "GRAMPIAN") ~ "Grampian",
      x %in% c("S08000008", "S08000022", "HG", "HIG", "HIGHLAND", "HIGHLANDS") ~ "Highland",
      x %in% c("S08000009", "S08000023", "S08000032", "LN", "LAN", "LANARKSHIRE") ~ "Lanarkshire",
      ...
      x %in% c("S08000013", "S08000027", "S08000030", "TY", "TAY", "TAYSIDE") ~ "Tayside",
      x %in% c("S08000014", "S08000028", "WI", "WESTERN ISLES") ~ "Western Isles",
      x %in% c("S08200001") ~ "England/Wales/Northern Ireland",
      x %in% c("S08200002") ~ "No Fixed Abode",
      x %in% c("S08200003") ~ "Not Known",
      x %in% c("S08200004") ~ "Outside U.K.",
